...
describe("Create new Custodial Task Type", () => {
  beforeEach(() => {
    cy.login(Cypress.env("user1"));
    cy.visit("/#!/Lookup/CDTaskType/Create");
  });

  it("Custodial Task Type - Negative Cases", { tags: "@smoke" }, () => {
  ...
  it(
    "Create Custodial Task Type with Required fields",
    { tags: "@smoke" },
    () => {
      cy.EditInputElement("Code", faker.datatype.number(999999));
      cy.EditInputElement("Description", faker.random.words(5));
  ...
  it("Create new Custodial Task Type Data with all fields", () => {
    cy.EditInputElement("Code", faker.datatype.number(999999));
    cy.EditInputElement("Description", faker.random.words(5));
