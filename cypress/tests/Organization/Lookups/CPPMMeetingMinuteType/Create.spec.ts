...
describe("Create CPPM Meeting Minute Type", () => {
  beforeEach(() => {
    cy.login(Cypress.env("user1"));
    cy.visit("/#!/Lookup/CJMeetingType/Create");
  });

  it("Meeting Minute Type -  Negative Cases", { tags: "@smoke" }, () => {
  ...
  it(
    "Create Meeting Minute Type with Required Fields",
    { tags: "@smoke" },
    () => {
      cy.EditInputElement("Code", faker.datatype.number(999999));
      cy.EditInputElement("Description", faker.random.words(5));
  ...
  it("Create New Meeting Minute Type with All fields", () => {
    cy.EditInputElement("Code", faker.datatype.number(999999));
    cy.EditInputElement("Description", faker.random.words(5));
