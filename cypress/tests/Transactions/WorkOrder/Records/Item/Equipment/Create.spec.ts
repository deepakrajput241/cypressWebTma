...
const data = {
  ...
  taskCode: "16546246",
  wOTypeDescription: "1 Corrective",
};

function fillRequiredFields() {
  cy.fillSelect("Item Type", "Equipment");
  cy.fillCombobox("Item Code", data.equipmentID);
  cy.fillTextarea("Request", faker.random.words(5));
  cy.fillCombobox("Task Code", data.taskCode);
}

describe("should create Work Order for Item of type Equipment", () => {
  beforeEach(() => {
    cy.login(Cypress.env("user1"));
    cy.visit("/#!/WorkOrder/Create");
  });

  it("should not create Work Order for Item of type Equipment when missing required fields", () => {
    // missing 'Item Code' (no fields required to be filled to get this validation error)
  ...
  it(
    "should create Work Order for Item of type Equipment with required fields, and then delete it",
    { tags: "@smoke" },
    () => {
      fillRequiredFields();
  ...
  // TODO: data changed - this test needs some updating
  it.skip("should create Work Order for Item of type Equipment with all fields, and then delete it", () => {
    cy.fillSelect("Item Type", "Equipment");
    cy.clearCombobox("Item Code");
    cy.fillCombobox("Item Code", data.equipmentID);
    cy.get("body[ng-controller='MainController as mainCtrl']").then(
      ($popup) => {
    ...
    cy.fillDateInput("Est. Start Date", new Date().toLocaleDateString("en-US"));
    cy.fillDateInput(
      "Est. End Date",
      faker.date.future().toLocaleDateString("en-US")
    );
